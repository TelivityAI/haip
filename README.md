...
GET    /api/v1/rate-plans/:id/effective-rate        # Calculate effective rate (?nights, ?checkIn, ?checkOut, ?stayDate)
PATCH  /api/v1/rate-plans/:id                      # Update rate plan (losAdjustments, occupancyBands)
...
GET    /api/v1/reports/pickup                       # Pickup report (?stayDate, ?from, ?to)
