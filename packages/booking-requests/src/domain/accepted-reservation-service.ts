 ...
 * `apps/api/src/common/accepted-pricing/accepted-reservation-service.ts` —
 * fully self-contained pure matching logic (only depends on the
 * `AcceptedPricingSnapshot` type this package already consumes), duplicated
 * here rather than imported so this package never imports from apps/api.
 */
