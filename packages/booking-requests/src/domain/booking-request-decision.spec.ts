...
import { PERMISSIONS_KEY } from '@telivityhaip/shared';
import {
...
describe('Booking Request staff HTTP contract', () => {
  it('registers concrete DTO validation and read/write permissions', async () => {
    const controllerModule = await import('../http/booking-request.controller.js').catch(() => null);
    const listDtoModule = await import('../http/dto/list-booking-requests.dto.js').catch(() => null);
    const acceptDtoModule = await import('../http/dto/accept-booking-request.dto.js').catch(() => null);
    const denyDtoModule = await import('../http/dto/deny-booking-request.dto.js').catch(() => null);
