...
async function main() {
  ...
  const tables = [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS reservations_property_id_unique ON reservations (property_id, id)`,
    `CREATE UNIQUE INDEX IF NOT EXISTS folios_property_id_unique ON folios (property_id, id)`,
    `CREATE TABLE IF NOT EXISTS booking_request_stay_amendments (
      ...
      price_source varchar(10) NOT NULL,
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS booking_request_stay_amendments_property_idempotency_unique ON booking_request_stay_amendments (property_id, idempotency_key)`,
    `CREATE UNIQUE INDEX IF NOT EXISTS br_stay_amendments_property_request_fingerprint_unique ON booking_request_stay_amendments (property_id, booking_request_id, operation_fingerprint)`,
  ...
  const alters = [
    ...
    `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_request_fkey') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_request_fkey
          FOREIGN KEY (property_id, booking_request_id) REFERENCES booking_requests(property_id, id);
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_reservation_fkey') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_reservation_fkey
          FOREIGN KEY (property_id, reservation_id) REFERENCES reservations(property_id, id);
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_folio_fkey') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_folio_fkey
          FOREIGN KEY (property_id, folio_id) REFERENCES folios(property_id, id);
      END IF;
    END $$`,
    `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_price_source_check') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_price_source_check
          CHECK (price_source IN ('prior', 'current', 'custom'));
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_dates_check') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_dates_check
          CHECK (new_departure_date > new_arrival_date);
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_total_check') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_total_check
          CHECK (new_total_amount > 0);
      END IF;
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'booking_request_stay_amendments_custom_reason_check') THEN
        ALTER TABLE booking_request_stay_amendments ADD CONSTRAINT booking_request_stay_amendments_custom_reason_check
          CHECK (price_source <> 'custom' OR (reason IS NOT NULL AND length(trim(reason)) > 0));
      END IF;
    END $$`,
    `WITH accepted_room_candidates AS (
      SELECT c.id, f.property_id, f.id AS folio_id, f.reservation_id,
      ...
      FROM folios f
      JOIN reservations r ON r.id = f.reservation_id AND r.property_id = f.property_id
      CROSS JOIN LATERAL jsonb_array_elements(
        CASE WHEN jsonb_typeof(r.accepted_pricing_snapshot->'nights') = 'array'
          THEN r.accepted_pricing_snapshot->'nights' ELSE '[]'::jsonb END
      ) AS night
      JOIN charges c ON c.folio_id = f.id AND c.property_id = f.property_id
        AND c.type = 'room' AND c.is_reversal = false
        AND c.parent_charge_id IS NULL AND c.source_key IS NULL
      ...
      SELECT candidate.* FROM accepted_room_candidates candidate
      WHERE candidate.candidate_rank = 1
        AND NOT EXISTS (
          SELECT 1 FROM charges claimed
