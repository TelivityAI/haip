...
# Metasearch via inPMS direct booking

inPMS does not ship a Google Hotel Ads / Tripadvisor / trivago adapter in-core.
...
2. **inPMS booking-engine deep links** as the landing destination for Free Booking Links / CPC programs you operate yourself.
...
Feeds must match what `/api/v1/booking-engine` returns for the same stay. Prefer CM-operated campaigns until inPMS is a registered Google Hotel Center partner.
