# Luxembourg fiches d\ — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh luxembourg-fiches
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `luxembourg-fiches`.
2. Sets property fiscal/guest-reg provider key to `luxembourg_fiches`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `luxembourg_fiches`.
