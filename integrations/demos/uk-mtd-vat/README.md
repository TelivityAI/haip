...
## What this demo does
1. Turns ON the Integrations catalog toggle for `uk-mtd-vat`.
2. Sets property fiscal/guest-reg provider key to `uk_mtd_vat`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `uk_mtd_vat`.
