# Portugal SIBA — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh portugal-siba
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `portugal-siba`.
2. Sets property fiscal/guest-reg provider key to `portugal_siba`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `portugal_siba`.
