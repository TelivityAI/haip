#!/usr/bin/env bash
# One-command demo: Spain VeriFactu (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Spain VeriFactu (spain-verifactu) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: spain_verifactu (console / demo handoff)"
require_api
enable_registry 'spain-verifactu'
put_fiscal '{"fiscalProviderKey":"spain_verifactu","documentType":"spain_verifactu","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (spain_verifactu) in fiscal config."
