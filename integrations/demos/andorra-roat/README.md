# Andorra ROAT — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh andorra-roat
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `andorra-roat`.
2. Sets property fiscal/guest-reg provider key to `andorra_roat`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `andorra_roat`.
