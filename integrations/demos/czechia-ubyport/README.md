# Czechia Ubyport — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh czechia-ubyport
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `czechia-ubyport`.
2. Sets property fiscal/guest-reg provider key to `czechia_ubyport`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `czechia_ubyport`.
