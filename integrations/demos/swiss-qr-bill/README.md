...
## What this demo does
1. Turns ON the Integrations catalog toggle for `swiss-qr-bill`.
2. Sets property fiscal/guest-reg provider key to `swiss_qr_bill`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `swiss_qr_bill`.
