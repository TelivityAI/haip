#!/usr/bin/env bash
# One-command demo: Poland KSeF (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Poland KSeF (poland-ksef) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: poland_ksef (console / demo handoff)"
require_api
enable_registry 'poland-ksef'
put_fiscal '{"fiscalProviderKey":"poland_ksef","documentType":"poland_ksef","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (poland_ksef) in fiscal config."
