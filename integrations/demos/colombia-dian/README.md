# Colombia DIAN — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh colombia-dian
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `colombia-dian`.
2. Sets property fiscal/guest-reg provider key to `colombia_dian`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `colombia_dian`.
