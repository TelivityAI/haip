# Belgium Peppol B2B — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh belgium-peppol-b2b
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `belgium-peppol-b2b`.
2. Sets property fiscal/guest-reg provider key to `belgium_peppol_b2b`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `belgium_peppol_b2b`.
