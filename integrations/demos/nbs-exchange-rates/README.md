# NBS exchange rates — adapter demo
...
## What this demo does
1. Turns ON the Integrations catalog toggle for `nbs-exchange-rates`.
2. Sets property fiscal/guest-reg provider key to `nbs_exchange_rates`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `nbs_exchange_rates`.
