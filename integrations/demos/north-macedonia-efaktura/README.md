# North Macedonia e-Faktura — adapter demo
...
## What this demo does
1. Turns ON the Integrations catalog toggle for `north-macedonia-efaktura`.
2. Sets property fiscal/guest-reg provider key to `north_macedonia_efaktura`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `north_macedonia_efaktura`.
