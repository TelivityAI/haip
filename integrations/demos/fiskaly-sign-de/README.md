...
## What this demo does
1. Turns ON the Integrations catalog toggle for `fiskaly-sign-de`.
2. Sets property fiscal/guest-reg provider key to `fiskaly_sign_de`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `fiskaly_sign_de`.
