# Romania RO e-Factura — adapter demo
...
## What this demo does
1. Turns ON the Integrations catalog toggle for `romania-ro-efactura`.
2. Sets property fiscal/guest-reg provider key to `romania_ro_efactura`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `romania_ro_efactura`.
