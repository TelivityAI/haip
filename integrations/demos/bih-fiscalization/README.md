# BiH fiscalization — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh bih-fiscalization
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `bih-fiscalization`.
2. Sets property fiscal/guest-reg provider key to `bih_fiscalization`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `bih_fiscalization`.
