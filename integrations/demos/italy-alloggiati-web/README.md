# Italy Alloggiati Web — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh italy-alloggiati-web
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `italy-alloggiati-web`.
2. Sets property fiscal/guest-reg provider key to `italy_alloggiati`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `italy_alloggiati`.
