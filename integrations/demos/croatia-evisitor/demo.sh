#!/usr/bin/env bash
# One-command demo: Croatia eVisitor (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Croatia eVisitor (croatia-evisitor) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: croatia_evisitor (console / demo handoff)"
require_api
enable_registry 'croatia-evisitor'
put_fiscal '{"guestRegistrationProviderKey":"croatia_evisitor","guestRegistrationConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (croatia_evisitor) in fiscal config."
