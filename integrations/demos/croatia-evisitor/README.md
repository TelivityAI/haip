# Croatia eVisitor — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh croatia-evisitor
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `croatia-evisitor`.
2. Sets property fiscal/guest-reg provider key to `croatia_evisitor`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `croatia_evisitor`.
