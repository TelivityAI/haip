...
## What this demo does
1. Turns ON the Integrations catalog toggle for `ticketbai`.
2. Sets property fiscal/guest-reg provider key to `spain_ticketbai`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `spain_ticketbai`.
