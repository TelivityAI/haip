...
```bash
./integrations/demos/run.sh nuki
# or:
./integrations/demos/nuki/demo.sh
```

What it does:
1. Turns **ON** the property Integrations catalog toggle for `nuki` (same as the dashboard button).
...
> Without credentials the lock factory falls back to console.
