# TripAdvisor Content API — demo

One command (API must be running — `docker compose up`):

```bash
./integrations/demos/run.sh tripadvisor-content-api
# or:
./integrations/demos/tripadvisor-content-api/demo.sh
```

What it does:
1. Turns **ON** the property Integrations catalog toggle for `tripadvisor-content-api` (same as the dashboard button).
