# Greece myDATA — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh greece-mydata
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `greece-mydata`.
2. Sets property fiscal/guest-reg provider key to `greece_mydata`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `greece_mydata`.
