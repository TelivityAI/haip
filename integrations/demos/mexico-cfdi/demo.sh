#!/usr/bin/env bash
# One-command demo: Mexico CFDI (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Mexico CFDI (mexico-cfdi) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: mexico_cfdi (console / demo handoff)"
require_api
enable_registry 'mexico-cfdi'
put_fiscal '{"fiscalProviderKey":"mexico_cfdi","documentType":"mexico_cfdi","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (mexico_cfdi) in fiscal config."
