# Mexico CFDI — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh mexico-cfdi
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `mexico-cfdi`.
2. Sets property fiscal/guest-reg provider key to `mexico_cfdi`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `mexico_cfdi`.
