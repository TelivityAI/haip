#!/usr/bin/env bash
# One-command demo: Slovenia FURS (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Slovenia FURS (slovenia-furs) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: slovenia_furs (console / demo handoff)"
require_api
enable_registry 'slovenia-furs'
put_fiscal '{"fiscalProviderKey":"slovenia_furs","documentType":"slovenia_furs","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (slovenia_furs) in fiscal config."
