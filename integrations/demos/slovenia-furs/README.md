# Slovenia FURS — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh slovenia-furs
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `slovenia-furs`.
2. Sets property fiscal/guest-reg provider key to `slovenia_furs`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `slovenia_furs`.
