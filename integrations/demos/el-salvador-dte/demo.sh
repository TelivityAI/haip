#!/usr/bin/env bash
# One-command demo: El Salvador DTE (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: El Salvador DTE (el-salvador-dte) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: el_salvador_dte (console / demo handoff)"
require_api
enable_registry 'el-salvador-dte'
put_fiscal '{"fiscalProviderKey":"el_salvador_dte","documentType":"el_salvador_dte","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (el_salvador_dte) in fiscal config."
