# El Salvador DTE — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh el-salvador-dte
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `el-salvador-dte`.
2. Sets property fiscal/guest-reg provider key to `el_salvador_dte`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `el_salvador_dte`.
