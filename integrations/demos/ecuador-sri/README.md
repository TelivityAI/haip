# Ecuador SRI — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh ecuador-sri
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `ecuador-sri`.
2. Sets property fiscal/guest-reg provider key to `ecuador_sri`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `ecuador_sri`.
