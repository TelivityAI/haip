#!/usr/bin/env bash
# One-command demo: Hungary NAV 3.0 (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Hungary NAV 3.0 (hungary-nav-3) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: hungary_nav_3 (console / demo handoff)"
require_api
enable_registry 'hungary-nav-3'
put_fiscal '{"fiscalProviderKey":"hungary_nav_3","documentType":"hungary_nav_3","fiscalConfig":{"demo":true}}'
echo "Note: Console adapter: logs handoff only. Live authority/partner client required for production filing."
print_live_hint
echo
echo "Go-live (not console):"
echo "  1. Obtain partner/authority credentials for this market."
echo "  2. Replace the console FiscalProvider/GuestRegistrationProvider with a real client."
echo "  3. Keep the same provider key (hungary_nav_3) in fiscal config."
echo "Docs: docs/integrations/wave3-fiscal-guest-reg.md"
echo "DONE."
