# Hungary NAV 3.0 — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh hungary-nav-3
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `hungary-nav-3`.
2. Sets property fiscal/guest-reg provider key to `hungary_nav_3`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `hungary_nav_3`.
