# Finland Matkustajailmoitus — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh finland-matkustajailmoitus
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `finland-matkustajailmoitus`.
2. Sets property fiscal/guest-reg provider key to `finland_matkustajailmoitus`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `finland_matkustajailmoitus`.
