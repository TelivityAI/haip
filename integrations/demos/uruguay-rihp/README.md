# Uruguay RIHP — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh uruguay-rihp
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `uruguay-rihp`.
2. Sets property fiscal/guest-reg provider key to `uruguay_rihp`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `uruguay_rihp`.
