#!/usr/bin/env bash
# One-command demo: Uruguay RIHP (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Uruguay RIHP (uruguay-rihp) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: uruguay_rihp (console / demo handoff)"
require_api
enable_registry 'uruguay-rihp'
put_fiscal '{"guestRegistrationProviderKey":"uruguay_rihp","guestRegistrationConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (uruguay_rihp) in fiscal config."
