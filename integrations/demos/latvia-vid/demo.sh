#!/usr/bin/env bash
# One-command demo: Latvia VID (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Latvia VID (latvia-vid) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: latvia_vid (console / demo handoff)"
require_api
enable_registry 'latvia-vid'
put_fiscal '{"fiscalProviderKey":"latvia_vid","documentType":"latvia_vid","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (latvia_vid) in fiscal config."
