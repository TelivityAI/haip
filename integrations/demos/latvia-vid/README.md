# Latvia VID — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh latvia-vid
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `latvia-vid`.
2. Sets property fiscal/guest-reg provider key to `latvia_vid`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `latvia_vid`.
