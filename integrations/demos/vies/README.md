...
```bash
./integrations/demos/run.sh vies
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `vies`.
2. Sets property fiscal/guest-reg provider key to `vies`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `vies`.
