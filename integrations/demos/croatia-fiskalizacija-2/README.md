# Croatia Fiskalizacija 2.0 — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh croatia-fiskalizacija-2
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `croatia-fiskalizacija-2`.
2. Sets property fiscal/guest-reg provider key to `croatia_fiskalizacija_2`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `croatia_fiskalizacija_2`.
