# Ireland VAT Modernisation — adapter demo
...
## What this demo does
1. Turns ON the Integrations catalog toggle for `ireland-vat-modernisation`.
2. Sets property fiscal/guest-reg provider key to `ireland_vat_modernisation`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `ireland_vat_modernisation`.
