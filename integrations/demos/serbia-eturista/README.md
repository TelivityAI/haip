# Serbia eTurista — demo

One command (API must be running — `docker compose up`):

```bash
./integrations/demos/run.sh serbia-eturista
# or:
./integrations/demos/serbia-eturista/demo.sh
```

What it does:
1. Turns **ON** the property Integrations catalog toggle for `serbia-eturista` (same as the dashboard button).
...
> Console guest-registration provider — logs handoff.
