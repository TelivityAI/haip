# Estonia e-invoicing — adapter demo
...
## What this demo does
1. Turns ON the Integrations catalog toggle for `estonia-e-invoicing`.
2. Sets property fiscal/guest-reg provider key to `estonia_einvoicing`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `estonia_einvoicing`.
