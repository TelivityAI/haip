# Infobip Omnichannel — demo

One command (API must be running — `docker compose up`):

```bash
./integrations/demos/run.sh infobip-omnichannel
# or:
./integrations/demos/infobip-omnichannel/demo.sh
```

What it does:
1. Turns **ON** the property Integrations catalog toggle for `infobip-omnichannel` (same as the dashboard button).
...
> Without Infobip credentials SMS falls through to console.
