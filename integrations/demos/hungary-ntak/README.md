# Hungary NTAK — adapter demo

**Status:** `adapter` (console handoff — not live authority filing)

```bash
./integrations/demos/run.sh hungary-ntak
```

## What this demo does
1. Turns ON the Integrations catalog toggle for `hungary-ntak`.
2. Sets property fiscal/guest-reg provider key to `hungary_ntak`.
...
2. Implement (or drop in) a real `FiscalProvider` / `GuestRegistrationProvider` registered under key `hungary_ntak`.
