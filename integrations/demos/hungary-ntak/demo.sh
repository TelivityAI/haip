#!/usr/bin/env bash
# One-command demo: Hungary NTAK (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: Hungary NTAK (hungary-ntak) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: hungary_ntak (console / demo handoff)"
require_api
enable_registry 'hungary-ntak'
put_fiscal '{"fiscalProviderKey":"hungary_ntak","documentType":"hungary_ntak","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (hungary_ntak) in fiscal config."
