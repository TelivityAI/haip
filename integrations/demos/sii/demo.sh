#!/usr/bin/env bash
# One-command demo: SII (console adapter)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
source "$ROOT/lib.sh"

echo "=== HAIP adapter demo: SII (sii) ==="
echo "API: $HAIP_URL  property: $PROPERTY_ID"
echo "Provider key: spain_sii (console / demo handoff)"
require_api
enable_registry 'sii'
put_fiscal '{"fiscalProviderKey":"spain_sii","documentType":"spain_sii","fiscalConfig":{"demo":true}}'
...
echo "  3. Keep the same provider key (spain_sii) in fiscal config."
