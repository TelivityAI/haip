...
export default function MediaGallery({ propertyId, ownerType, ownerId, canManage = true }: Props) {
  ...
  return (
    <div>
      {isLoading ? (
        <p className="text-sm text-telivity-mid-grey">{t('media.loadingImages')}</p>
      ) : items.length === 0 ? (
            ...
            <div key={m.id} className="group relative rounded-xl overflow-hidden border border-gray-200 bg-gray-50">
              ...
              {m.isPrimary && (
                <span className="absolute top-1.5 left-1.5 flex items-center gap-1 bg-telivity-teal text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">
                  <Star size={10} fill="currentColor" /> {t('media.primary')}
                </span>
              )}
              {m.caption && (
              ...
              {canManage && (
                <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {!m.isPrimary && (
                    <button title={t('media.setAsPrimary')} onClick={() => setPrimary.mutate(m.id)} className="p-1 rounded bg-white/90 hover:bg-white text-telivity-slate">
                      <Star size={13} />
                    </button>
                  )}
                  <button title={t('media.delete')} onClick={() => remove.mutate(m.id)} className="p-1 rounded bg-white/90 hover:bg-white text-red-500">
                    <Trash2 size={13} />
                  </button>
                </div>
              )}
              {canManage && items.length > 1 && (
                <div className="absolute bottom-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button title={t('media.moveLeft')} disabled={i === 0} onClick={() => move(i, -1)} className="p-1 rounded bg-white/90 hover:bg-white text-telivity-slate disabled:opacity-30">
                    <ArrowLeft size={13} />
                  </button>
                  <button title={t('media.moveRight')} disabled={i === items.length - 1} onClick={() => move(i, 1)} className="p-1 rounded bg-white/90 hover:bg-white text-telivity-slate disabled:opacity-30">
                    <ArrowRight size={13} />
                  </button>
                </div>
              )}
      ...
      {canManage && (
        <div className="mt-4 border-t border-gray-100 pt-4 space-y-2">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <LinkIcon size={14} className="text-telivity-slate shrink-0" />
            <input
              type="url"
              value={url}
            ...
            <button
              ...
              className="w-full sm:w-auto bg-telivity-deep-blue text-white rounded-lg px-4 py-2 text-sm font-semibold disabled:opacity-50 shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-telivity-deep-blue focus-visible:ring-offset-2"
            >
              {t('media.add')}
            </button>
          </div>
